...
public class state2 extends JPanel implements ActionListener {

    public ImageIcon imgstate1 = new ImageIcon(this.getClass().getResource("state2.png"));
    ...
    public ArrayList<zubat> zu = new ArrayList<zubat>();
    ...
    Thread zubat = new Thread(new Runnable() {
        public void run() {
            while (true) {
                try {
                    if (st == false) {
        ...
        if (times <= 0 || eg <= 0) {
            ...
            for (int i = 0; i < et2.size(); i++) {
                for (int j = 0; j < zu.size(); j++) {
                    if (Intersect(et2.get(i).getbound(), zu.get(j).getbound())) {
                        zu.remove(j);
                        et2.remove(i);
                        sc += 10;
            ...
            for (int i = 0; i < et.size(); i++) {
                for (int j = 0; j < zu.size(); j++) {
                    if (Intersect(et.get(i).getbound(), zu.get(j).getbound())) {
                        zu.remove(j);
                        et.remove(i);
                        sc += 10;
            ...
            for (int i = 0; i < zu.size(); i++) {
                g.drawImage(zu.get(i).getImage(), zu.get(i).getX(),zu.get(i).getY(),300, 300, this);
                if (Intersect(zu.get(i).getbound(), p.getbound())) {
                    eg -= 1;
                    zu.remove(i);
                }
            }
